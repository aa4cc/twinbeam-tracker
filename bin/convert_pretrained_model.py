...
def convert(mat_path):
    ...
    for i in tqdm(range(netparams.size)):
        ...
        match = re.match(r"br_([a-z]+)([0-9]+)([a-z]+)", name, re.I)
        if match:
            items = match.groups()
        elif name == 'adjust_f':
            continue
        elif name == 'fin_adjust_bnm':
        ...
        if layer in [1, 2, 3, 4, 5]:
            ...
            if op == 'conv':  # convolution
                if types == 'f':
                    ...
                    if value.shape == (1, 1) or value.shape == (1, 2):
                        continue
                    if value.shape[-1] == 1:
                        value = squeeze(value)
...
if __name__ == '__main__':
    path = 'models/baseline-conv5_e55'
    params = convert(path + '.mat')
    torch.save(params, path + ".pth")
